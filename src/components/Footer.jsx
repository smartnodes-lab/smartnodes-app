...
const Footer = () => (
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col px-5 md:px-10 border-gray dark:border-white border-t-[1px] border-t-[#3F3E45]`}>
      ...
      <div className="flex-[1.5] w-full flex flex-row justify-between flex-wrap md:mt-0 mt-10">
        {footerLinks.map((footerlink) => (
          <div key={footerlink.title} className={`flex flex-col ss:my-0 my-4 min-w-[150px]`}>
            <h4 className="font-poppins font-medium text-[18px] leading-[27px] text-grey-700 dark:text-white">
              {footerlink.title}
            </h4>
            <ul className="list-none mt-4">
              {footerlink.links.map((link, index) => (
                <li
                  key={link.name}
                  onClick={footerlink.title === "Useful Links" | link.name === "Buy us a Coffee" ? () => window.open(link.link) : undefined}
    ...
    <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]">
      <p className="font-poppins font-normal text-center text-[18px] leading-[27px] dark:text-white text-grey-700">
        Copyright Ⓒ 2024 Smartnodes. All Rights Reserved.
      </p>
